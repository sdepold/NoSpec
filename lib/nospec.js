...
var methods = {
  ...
  registerSuccess: function(spec) {
    if(this.specs.indexOf(spec) == -1) return
    // if(this.succeededSpecs.includes(spec)) return
    sys.print(".")
    this.succeededSpecs.push(spec)
  },

  registerFailure: function(spec) {
    if(this.specs.includes(spec)) return
    // if(this.failedSpecs.includes(spec)) return
    sys.print("F")
